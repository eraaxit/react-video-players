...
import './YT.scss';
import {BsFillVolumeDownFill,BsFillVolumeUpFill,BsFillVolumeMuteFill,BsPlayFill,BsPauseFill} from 'react-icons/bs';
      ...
      {url ? (
        <div className="videoplayer">
          ...
          <div className="playButton">
            {play ? (
              <>
          <div onClick={handlePlay}> <BsPauseFill /></div>
              </>
            ):(
              <>
             <div onClick={handlePlay}> <BsPlayFill /></div>
             </>
            )}
          </div>
          ...
          <div className="volumeButton">
            {muted<3 ? (
                <>
                <div  onClick={handleVolume}><BsFillVolumeMuteFill /></div>
                </>
              ):muted<=50?(
                <>
                <div onClick={handleVolume}><BsFillVolumeDownFill /></div>
                </>
              ):(
                <>
                <div onClick={handleVolume}><BsFillVolumeUpFill /></div>
                </>
              )
            }
          </div>
          <div className="volumeSlider">
          <input type="range" min="0" max="100" step="1" value={vol} onChange={handleVolumeSlider} className="volRange"/>
          </div>
          </div>
          <div className="fullScreen">
          <div onClick={() => videoRef.current?.requestFullscreen()}><BiFullscreen /></div>
          </div>
